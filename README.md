...
Extra
🔽 1. Download MPJ Express again

Download from the official source:

MPJ Express 0.44: 👉 https://sourceforge.net/projects/mpjexpress/files/latest/download

Or run this in terminal to download directly:
...
This will create a folder like mpj-v0_44.

✅ Now verify that lib/mpj.jar exists:

ls ~/Downloads/mpj-v0_44/lib/mpj.jar

You should see the JAR file listed.
🔧 3. Set environment variables again (temporarily)
...
cd ~/Downloads/DS-main/MPI/MPI_ADD
javac -cp $MPJ_HOME/lib/mpj.jar Ass.java
$MPJ_HOME/bin/mpjrun.sh -np 4 Ass

💡 Optional: Make environment permanent

To avoid doing this every time, add to ~/.bashrc:

echo 'export MPJ_HOME=~/Downloads/mpj-v0_44' >> ~/.bashrc
...
export PATH=$MPJ_HOME/bin:$PATH
javac -cp $MPJ_HOME/lib/mpj.jar ArrSum.java
$MPJ_HOME/bin/mpjrun.sh -np 3 ArrSum
...
$MPJ_HOME/bin/mpjrun.sh -np 4 ArrMul

export MPJ_HOME=~/Downloads/mpj-v0_44
export PATH=$MPJ_HOME/bin:$PATH
javac -cp $MPJ_HOME/lib/mpj.jar Reciprocal.java
$MPJ_HOME/bin/mpjrun.sh -np 4 Reciprocal


export MPJ_HOME=~/Downloads/mpj-v0_44
export PATH=$MPJ_HOME/bin:$PATH
javac -cp $MPJ_HOME/lib/mpj.jar Average.java
$MPJ_HOME/bin/mpjrun.sh -np 4 Average
